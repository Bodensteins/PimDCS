...
class PimAlexNet_OU(nn.Module):
    def __init__(self):
        ...
        self.alexnet = nn.Sequential(
            Conv2d_OU(3, 64, kernel_size=(3, 3), stride=1, padding=(1, 1)),  # CIFAR-10 的图像为 32x32, 因此使用 3x3 卷积核
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),

            Conv2d_OU(64, 192, kernel_size=(3, 3), padding=(1, 1)),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),

            Conv2d_OU(192, 384, kernel_size=(3, 3), padding=(1, 1)),
            nn.ReLU(inplace=True),

            Conv2d_OU(384, 256, kernel_size=(3, 3), padding=(1, 1)),
            nn.ReLU(inplace=True),

            Conv2d_OU(256, 256, kernel_size=(3, 3), padding=(1, 1)),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            
            nn.Flatten(),
            nn.Dropout(),
            Linear_OU(256 * 4 * 4, 1024),  # 根据 CIFAR-10 图像大小调整全连接层的输入维度
            nn.ReLU(inplace=True),
            ...
            Linear_OU(512, 10),  # 最后一层的输出节点数为类别数（10）
        )

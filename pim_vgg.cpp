...
struct VGG : torch::nn::Module {
  VGG() {
    std::vector<int> in_channels = {3, 64, 64, 128, 128, 256, 256, 256, 512, 512, 512, 512, 512, 512};
    std::vector<int> in_shape = {32, 32, 16, 16, 8, 8, 8, 4, 4, 4, 2, 2, 2};
    for (int i = 0; i < 13; i++) {
      conv_list.push_back(PIM::PimConv2d(
          ExpandingArray<4>({kTrainBatchSize, in_channels[i], in_shape[i], in_shape[i]}),
          PIM::PimArrayType::simple_logic_array,
          Conv2dOptions(in_channels[i], in_channels[i+1], {3, 3}).padding({1, 1})
      ));
      register_module("Conv2d"+to_string(i+1), conv_list[i]);
    }

    for (int i = 0; i < 14; i++) {
      max_pooling_list.push_back(torch::nn::MaxPool2d({2, 2}));
      register_module("MaxPool2d"+to_string(i+1), max_pooling_list[i]);
    }

    for (int i = 0; i < 14; i++) {
      bn_list.push_back(torch::nn::BatchNorm2d());
  ...
  std::vector<PIM::PimConv2d> conv_list;

...
=======
        ...
        # print(de_quantization([wtArr, wtArr_cfg]).t())
>>>>>>> 9d2db88e22ce614e73a428fa0f84e454cc41e74d

        state = self.state[wArr]

        # merge
        if weight_decay != 0:
          add_alpha_tensor_([d_wt, d_wt_cfg], [wtArr, wtArr_cfg], weight_decay)
        if momentum != 0:
          if 'momentum_buffer' not in state:
            ...
            add_alpha_tensor_([buf, d_wt_cfg], [d_wt, d_wt_cfg], 1 - dampening)
          if nesterov:
            add_alpha_tensor_([d_wt, d_wt_cfg], [buf, d_wt_cfg], momentum)
          else:
            d_wt = buf

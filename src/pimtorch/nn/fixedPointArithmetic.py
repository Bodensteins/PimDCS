...
from src.pimtorch.config.globalCfg import TensorType, RightShiftMode, WeightUpdateStrategy, globalCfg

def to_float(tensor):
    return tensor.view(dtype=globalCfg.torchFloat)


def to_int(tensor):
    return tensor.detach().view(dtype=globalCfg.torchInt)
...
def _matmul_int_cuda(int_tensor1, int_tensor2):
    ...
    float_tensor1 = int_tensor1.to(dtype=globalCfg.torchFloat)
    float_tensor2 = int_tensor2.to(dtype=globalCfg.torchFloat)
    mul_result = float_tensor1.matmul(float_tensor2)

    return mul_result.to(dtype=globalCfg.torchInt)
...
def _round_rshift_(int_tensor, shift: int):
    ...
    elif shift > globalCfg.systemBitWidth - 1:
        int_tensor.zero_()
    else:
...
def _round_rshift(int_tensor, shift: int):
    ...
    if shift > globalCfg.systemBitWidth - 1:
...
def _round_to_nearest_even_rshift(int_tensor, shift: int):
    # IEEE754 round
    if shift <= 0 or shift > globalCfg.systemBitWidth - 1:
        raise Exception("Inappropriate shift value: " + str(shift))
...
def _creat_quantization_para(s: int = None, tensor_type: TensorType = None, device=None):
    ...
    """
    quantization_para = torch.empty(2, dtype=globalCfg.torchInt, device=device)
    if s is not None:
...
def quantization_tensor(tensor: Tensor, bit_width: int, tensor_type: TensorType):
    ...
    resolution = pow(2, s)
    if bit_width == 1:
        int_tensor = tensor.less(0).to(globalCfg.torchInt).mul(-1)
    elif tensor_type == TensorType.Normal or tensor_type == TensorType.PN:
        int_tensor = tensor.div(resolution).round().to(globalCfg.torchInt)
    else:
        raise Exception("Unknown tensor type: " + str(tensor_type.value))
...
def de_quantization(fp_tensor_tuple: tuple) -> Tensor:
    ...
    if tensor_type == TensorType.Normal or tensor_type == TensorType.PN:
        return int_tensor.to(globalCfg.torchFloat).mul(resolution)
    else:
        raise Exception("Unknown tensor type: " + str(tensor_type.value))
...
def add_additional_col_of_one(fp_tensor_tuple: tuple):
    ...
    if decimal_part_bit_width < 0:
        ...
        if decimal_part_bit_width > globalCfg.dataFlowBitWidth - 2:
            right_shift = decimal_part_bit_width - (globalCfg.dataFlowBitWidth - 2)
            ...
            decimal_part_bit_width = globalCfg.dataFlowBitWidth - 2

        int_one = 1 << decimal_part_bit_width

    full_one_col = torch.full([int_tensor.size()[0], 1], int_one, dtype=globalCfg.torchInt, device=int_tensor.device)
...
def fixed_point_matmul(input_tensor_tuple: tuple, other_tensor_tuple: tuple,
                       result_bit_width=globalCfg.dataFlowBitWidth) -> [Tensor, Tensor]:
...
def fixed_point_mul(tensor_tuple: tuple, alpha: float, alpha_bit_width: int = globalCfg.dataFlowBitWidth) -> [Tensor, Tensor]:
    int_tensor, quantization_para = _parse_tensor_tuple_to_int(tensor_tuple)
    s, tensor_type = _parse_quantization_para(quantization_para)
    ...
    set_bit_width_((mul_num_int_tensor, mul_num_para), globalCfg.dataFlowBitWidth)
    return mul_num_int_tensor, mul_num_para


def fixed_point_mul_(tensor_tuple: tuple, alpha: float, alpha_bit_width: int = globalCfg.dataFlowBitWidth) -> [Tensor, Tensor]:
    int_tensor, quantization_para = _parse_tensor_tuple_to_int(tensor_tuple)
    _, tensor_type = _parse_quantization_para(quantization_para)
    ...
    set_bit_width_(tensor_tuple, globalCfg.dataFlowBitWidth)
    return tensor_tuple


# todo: modify its implementation, have bugs
def fixed_point_add_(source_tensor_tuple: tuple, other_tensor_tuple: tuple, source_bit_width: int = globalCfg.dataFlowBitWidth,
                     alpha: float = None, alpha_bit_width: int = globalCfg.dataFlowBitWidth,
    ...
    if source_tensor_type == TensorType.Normal or TensorType.PN:
        if shift > 0:
            ...
            if mul_num_bit_width - shift > globalCfg.systemBitWidth:

...
class FpDataAnalyzer:
    ...
    def update_input_sparsity(self) -> None:
        ...
        # 更新输入分片稀疏度列表
        for i in range(self.mm_manager.weight_bit_width):
            self.input_bit_slices_sparsity[i] += zero_ratio[i].item()

    # 统计OU粒度大小输入向量的出现频率
    # 是否应该让unique和counts一直留在cuda上？
    def update_bit_vec_counts(self) -> None:
    ...
    def update_bit_vec_ones_num(self) -> None:
        assert self.mm_manager.fp_input_split != None
        input_split = self.mm_manager.fp_input_split
        # input_split维度大小分别为[ou_row_num, batch_size*bit_width, ou_row_size]
    ...
    def print_statistic(self):
        ...
        print("\n输入比特分片稀疏度:")
        for i, sparsity in enumerate(self.input_bit_slices_sparsity):
            print(f"bit {i}, sparsity:{sparsity:.2%}")
        print("\nOU粒度输入向量比特出现次数和频率:")
        for i, counts_dict in enumerate(self.bit_vec_counts_dict_list):
        ...
        print("\n不同1数量的输入向量的出现次数和频率:")
        total_counts = sum(self.bit_vec_ones_num_dict.values())
        print(self.bit_vec_ones_num_dict)
        for ones_num in self.bit_vec_ones_num_dict:
            counts = self.bit_vec_ones_num_dict[ones_num]
            propotion = counts / total_counts

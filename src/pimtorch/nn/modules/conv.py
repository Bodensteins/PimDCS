...
from src.pimtorch.nn.modules.quan import dequan, quan
from src.pimtorch.nn.modules.linear import linear
...
class Conv2d(Module):
    ...
    def forward(self, _input: Tensor):
        ...
        fp_input, fp_input_cfg = quan.apply(_input, self.inputBits)

        # re-use linear function to get the answer
        qoutput, qoutput_config \
            = linear.apply(fp_input, fp_input_cfg, self.fp_weight, self.fp_weight_cfg, self.hasBias,
        ...
        output = dequan.apply(qoutput, qoutput_config, self.nextGradOutputBits)

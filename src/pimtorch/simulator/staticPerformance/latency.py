...
from src.pimtorch.simulator.staticPerformance.utils import int_div_ceil
from examples.vgg_cifar10_model import fp_vgg16
...
class latencyModule:
    def __init__(self, net: nn.Module):
        ...
        self.iterations = (cfg.dataSize + cfg.batchSize - 1) // cfg.batchSize
        self.training = cfg.training
        self.compute_mode = cfg.computeMode
        ...
        self.activation_unit_num = cfg.activationUnitNum
        self.activation_unit_latency = cfg.activationUnitLatency

        self.all_latency = 0.0
        self.stage_latency_list = self.produce_stage_latency_list()
        self.normal_iter_compute_latency = self.calculate_iter_compute_latency(cfg.batchSize)
        self.last_iter_compute_latency = self.calculate_iter_compute_latency(cfg.dataSize % cfg.batchSize)
...
def test():
    bit_width_tuple = json.loads(cfg.config["Custom"]["BitWidthOfLayers"])

...
from src.pimtorch import nn as fpnn
from src.pimtorch.config.globalCfg import globalCfg as cfg
from src.pimtorch.config.globalCfg import PerformanceEvaluateMode as EvalMode
from src.pimtorch.simulator.staticPerformance.areaModule import AreaModule
from src.pimtorch.simulator.mapping.allocator import Allocator
from src.pimtorch.simulator.mapping.logicalTensor import LogicalTensor
from src.pimtorch.simulator.addressParser import ArchLevel
from examples.mnist_model import ConvMnist, FcMnist, PimFcMnist, PimConvMnist
...
class PerformanceManager:
  def __init__(self, net: nn.Module, resManager, mappingTable):
    """
    net: nn.Module
    ...
    self.input_data_shape = cfg.inputDataShape
    self.bit_width = cfg.weightBitWidth
    self.run_mode = cfg.performanceEvaluateMode
    self.array_mode = cfg.arrayMode

    self.layer_record = {}
    self.net_map_times = cfg.netMapTimes

    # map the nn to LogicArray and PhysicalArray
    self.layer_type_list, self.data_shape_list = self.map_network(net)

    # init staticPerformance module
    self.areaModule = AreaModule(self.resManager)
  ...
  def produce_layer_config(self, layer_with_weight_index):
    if self.net_map_times is not None:
      if len(self.net_map_times) <= layer_with_weight_index:
        raise Exception(
          "Illegal length of net map times list! list length: " + str(len(self.net_map_times)) + ", layer index: "
  ...
  def _map_layer_with_weight(self, layer_name: str, layer: nn.Module, layer_index=0) -> None:
    ...
    for i in range(pn_replication_degree * layer_config.weight_replication_degree):
      ...
      self.mappingTable.addLogicalTensor(
          LogicalTensor(size=(_in, _out), name=array_name, resourceManager=self.resManager,
                        mappingTable=self.mappingTable, allocator=Allocator, granularity=ArchLevel.crx)
      )
      layer_config.array_names.append(array_name)

    # running mode
    if self.run_mode != EvalMode.Inference:
      # training mode
      for i in range(pn_replication_degree * layer_config.weight_replication_degree):
        array_name = f"{layer_name}_{array_mode}{i % pn_replication_degree}_copy{i // pn_replication_degree}_backward"
        self.mappingTable.addLogicalTensor(
            LogicalTensor(size=(_out, _in), name=array_name, resourceManager=self.resManager,
                          mappingTable=self.mappingTable, allocator=Allocator, granularity=ArchLevel.crx)
        )
        layer_config.array_names.append(array_name)
      ...
      if children_num == 0:  # leaf node
        if isinstance(module, fpnn.Linear):
          ...
          if len(data_shape) != 2 or data_shape[0] != 1 or data_shape[1] != module.in_features:
            raise Exception(
              "illegal shape, input data shape: " + str(data_shape) + ", but in features: " + str(module.in_features))
...
if __name__ == "__main__":
  ...
  performanceManager = PerformanceManager(net, resManager, mappingTable)
  performanceManager.areaModule.printArch()
  performanceManager.areaModule.print()

  for layer_name, layer_cfg in performanceManager.layer_record.items():
    print(layer_name)
    print(layer_cfg.array_names)
    for array_name in layer_cfg.array_names:
      print(mappingTable.getLogicalTensor(tensorName=array_name))

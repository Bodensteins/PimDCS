...
public:
  ...
  Darknet(YAML::Node &config, const pim_array_pro_config* pim_cfg, torch::Device &device);
...
struct DetectionLayer : torch::nn::Module {
  ...
  torch::Tensor predict_transform(torch::Tensor prediction, int inp_dim, vector<float> anchors, int num_classes,
                                  torch::Device device) {
...
// Darknet
//---------------------------------------------------------------------------
Darknet::Darknet(YAML::Node &config, const pim_array_pro_config* pim_cfg, torch::Device &device)
...
//          conv_options(prev_filters, filters, kernel_size, stride, pad, 1, with_bias));
      ...
      PIM::PimConv2d pim_conv = PIM::PimConv2d(
          ExpandingArray<4>(in_shape), PIM::PimArrayType::pim_array_pro, pim_cfg,
      ...
      if (batch_normalize > 0) {
        torch::nn::BatchNorm2dImpl bn = torch::nn::BatchNorm2dImpl(
            bn_options(filters));  //@ihmc3jn09hk Fix for PyTorch 1.6
        module->push_back(bn);
      }
...
torch::Tensor Darknet::write_results(torch::Tensor prediction, int num_classes, float confidence, float nms_conf) {
  ...
  for (int i = 0; i < batch_size; i++) {
    ...
    for (size_t k = 0; k < img_classes.size(); k++) {
      ...
      auto cls_mask =
          image_prediction_data * (image_prediction_data.select(1, 6) == cls).to(torch::kFloat32).unsqueeze(1);
